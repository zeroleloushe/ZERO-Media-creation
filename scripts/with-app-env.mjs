...
function withLocalBin(processEnv) {
...
/** Resolve a local npm bin so Windows does not need `vite` on PATH. */
function resolveCommand(command) {
  if (command === "vite") {
    const js = join(projectRoot(), "node_modules", "vite", "bin", "vite.js");
    if (existsSync(js)) return { cmd: process.execPath, extra: [js], shell: false };
  }
  ...
  for (const c of candidates) {
    if (existsSync(c)) return { cmd: c, extra: [], shell: win && c.endsWith(".cmd") };
  }
  return { cmd: command, extra: [], shell: win };
}
...
function main(argv) {
  ...
  const { cmd, extra, shell } = resolveCommand(command);
